...
def fetch_card_image(card_name=None, set_code=None, collector_number=None):
    ...
    if set_code and collector_number:
        ...
        response = requests.get(direct_url, timeout=REQUEST_TIMEOUT_SECONDS)
    ...
    if (not data or ('image_uris' not in data and 'card_faces' not in data)) and card_name:
        ...
        response = requests.get(fallback_url, timeout=REQUEST_TIMEOUT_SECONDS)
    ...
    if layout in ['transform', 'modal_dfc', 'double_faced_token', 'flip', 'split', 'reversible_card']:
        if 'card_faces' in data:
            ...
            if front_image_url:
                front_response = requests.get(front_image_url, timeout=REQUEST_TIMEOUT_SECONDS)
                if front_response.status_code == 200:
                    front_image = Image.open(BytesIO(front_response.content))

            if back_image_url:
                back_response = requests.get(back_image_url, timeout=REQUEST_TIMEOUT_SECONDS)
            ...
            if 'image_uris' in data:
                ...
                if single_url:
                    single_resp = requests.get(single_url, timeout=REQUEST_TIMEOUT_SECONDS)
